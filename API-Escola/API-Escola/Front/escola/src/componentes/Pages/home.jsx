...
export default function Home() {
    ...
    const atualizar = async (ProfessorAtualizado) => {
        try {
            ...
            setDados(dados.map((Professor)=> professorSelecionado.id === ProfessorAtualizado.id ? ProfessorAtualizado : Professor)); //Transforma em tabela e procura dado por dado colocando um parametro
            setmodalOpen(false); 
        } catch (error) {
        ...
        <>
            ...
            <table>
                ...
                <tbody>
                    {dados.map((dado) => (
                        <tr key={dado.ni}>
                            <td><FaTrash className="icon" onClick={() => apagar(dado.id)} />  <MdCreate className="icon" onClick={() => atualizar(professorSelecionado)} /></td>
                            <td>{dado.ni}</td>
                            <td>{dado.nome}</td>
                            <td>{dado.email}</td>
                            <td>{dado.tel}</td>
